...
export function Install() {
  return (
    <div className="mx-auto max-w-3xl px-6 py-16">
      <Reveal as="h1" className="mb-3 text-2xl font-semibold text-white">
        Installation
      </Reveal>
      <Reveal delayMs={80} as="p" className="mb-10 text-sm text-slate-400">
      ...
      <div className="space-y-8">
        {STEPS.map((step, i) => (
          <Reveal key={step.title} delayMs={i === 0 ? 160 : 0}>
            <h2 className="mb-2 text-sm font-semibold text-white">{step.title}</h2>
            <pre className="overflow-x-auto rounded-lg border border-slate-800 bg-slate-950 p-4 text-xs text-slate-300 transition-colors duration-200 ease-out hover:border-slate-700">
              {step.code}
            </pre>
